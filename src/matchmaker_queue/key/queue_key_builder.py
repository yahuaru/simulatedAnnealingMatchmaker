from matchmaker_queue.key.queue_key_by_level import ByLevelQueueKeyCondition
...
class QueueKeyBuilder(object):
    ...
    def __init__(self, params):
        ...
        for battle_type, battle_param in params.items():
            self._key_builders[battle_type] = []
            ...
            for queue_key_builder in QueueKeyBuilder.QUEUE_KEY_BUILDERS:
                if queue_key_builder.REQUIRED_FIELDS.issubset(param_keys):
                    self._key_builders[battle_type].append(queue_key_builder(battle_param))

    def get_division_key(self, battle_type, division):
        description_key = []
        for queue_key_builder in self._key_builders[battle_type]:
